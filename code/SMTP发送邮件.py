...
# msg: 发送消息
# 这里要注意一下第三个参数，msg是字符串，表示邮件。我们知道邮件一般由标题，发信人，收件人，邮件内容，附件等构成，发送邮件的时候，要注意msg的格式。
# 这个格式就是smtp协议中定义的格式。
...
# 使用其他服务商的 SMTP 访问（QQ、网易、Google等）
import smtplib
from email.mime.text import MIMEText
from email.header import Header
...
# 邮件的 HTML 文本中一般邮件服务商添加外链是无效的，正确添加突破的实例。
import smtplib
...
# 参考 https://docs.python.org/3/library/email-examples.html